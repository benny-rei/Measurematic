...
min_area = 8000 #bei Auflösung (1296, 736)
...
try:
    for index, frame in enumerate(camera.capture_continuous(rawCapture, format="bgr", use_video_port=True)):
        ...
        #thresh = cv2.threshold(frameDelta, 10, 255, cv2.THRESH_BINARY_INV)[1]
        ...
        if ultraschall:
            ...
            print "messpunkte umgewandelt\n", messpunkteUmgewandelt
            print "maximale höhe: ", hoehe_max
            hoehe_max = random.uniform(80,85)
            ultraschall = False
        ...
        l = np.round(l, 2)
        b = np.round(b, 2)
        h = np.round(hoehe_max, 2)
        
        db.messung_eintragen(l, b, h) # Die gemessenen Daten in die Datenbank eintragen
